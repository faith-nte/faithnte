...
function transformWordPressPost(wpPost: WordPressPost): BlogPost {
  ...
  const tags = wpPost._embedded?.['wp:term']?.[0]?.filter(term => term.taxonomy === 'post_tag')?.map(tag => tag.slug) || [];
...
async function fetchWordPressPosts(): Promise<BlogPost[]> {
  try {
    const response = await fetch(`${WORDPRESS_API_URL}?_embed=true&per_page=100&status=publish`, {
      headers: {
...
export async function getPaginatedPosts(page: number = 1, limit: number = 10): Promise<PaginatedBlogPosts> {
